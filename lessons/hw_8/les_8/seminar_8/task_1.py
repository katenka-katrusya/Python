...
def info(direct: Path):
    json_data = {}
    fieldnames = ['name', 'path', 'size', 'file_or_dir']
    rows = []
    with open('info.json', 'w') as f_json, open('info.csv', 'w', newline='', encoding='utf-8') as f_csv,\
            open('info.pickle', 'wb') as f_pickle:
